...
| ⚠️ 网易云歌曲页（music.163.com/song?id=）采集卡死超 10 分钟零输出（v1.23.3 压测实锤，未根治） | iframe 渲染页疑似某步无界等待（进程低 CPU 纯等待）。现状：Ctrl+C 终止；改用 `--method ytdlp` 直下或人工浏览器下载。音频电路的"登录页抢位"已修（不会把 music.163.com/login 当目标）；工具侧根治需加全局 `--timeout` 看门狗，列入待办 |
