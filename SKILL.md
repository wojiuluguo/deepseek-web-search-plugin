...
| 防"无限挂死"（AI Agent 必看） | 任意命令加 `--timeout 600`（v1.24.0 全局看门狗：chain 各步有界但总和无界，加了这条整条命令必然结束，超时打 [timeout] 标记 exit 1） |
...
| ⚠️ 网易云歌曲页（music.163.com/song?id=）采集卡死超 10 分钟零输出（v1.23.3 压测实锤） | iframe 渲染页疑似某步无界等待。**v1.24.0 起有解：任意命令加 `--timeout 600`（全局看门狗，整条命令 N 秒必然结束，exit 1 + [timeout] 标记）**；或改用 `--method ytdlp` 直下。音频电路的"登录页抢位"已修（不会把 music.163.com/login 当目标）。AI Agent 调用建议**统一带 `--timeout 600`** |
